...
<div id="bcek-editor-wrapper" class="bcek-user-container">
    ...
    <div class="bcek-user-container-wrapper">
        <main class="bcek-preview-area">
            <!-- Adicionamos position: relative para garantir que o canvas cubra a área -->
            <div id="bcek-canvas-container" style="position: relative;">
                <!-- Adicionamos opacity: 0; e consertamos a tag img -->
                <img id="bcek-base-image-preview" src="<?php echo esc_url( $template->base_image_url ); ?>" alt="Pré-visualização" style="opacity: 0; width: 100%; height: auto;">
                
                <!-- Adicionamos CSS absoluto para o canvas cobrir a imagem perfeitamente -->
                <canvas id="bcek-text-overlay-canvas" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;"></canvas>
            </div>
        </main>
                                    ...
                                    <div class="accordion-content-inner">
                                        <div class="bcek-input-group space-y-2" data-field-id="<?php echo esc_attr( $field->field_id ); ?>" data-field-type="<?php echo esc_attr( $field->field_type ?? 'text' ); ?>">
                                            ...
                                            <?php if ( $field->field_type === 'image' ) : ?>
                                                <div class="bcek-file-input-wrapper">
                                                    ...
                                                    <label for="bcek_field_<?php echo esc_attr( $field->field_id ); ?>" class="bcek-custom-file-upload">
                                                        Escolher Arquivo
                                                    </label>
                                            
                                                    <span class="bcek-file-name">Nenhum arquivo escolhido</span>
                                                </div>
                                            ...
                                            <?php else : // --- BLOCO DE TEXTO MODIFICADO --- ?>
                                                ...
                                                <?php
                                                    ...
                                                    if ( ! empty( $default_text ) ) {
                                                        // 1. Normaliza as quebras de linha para o padrão
                                                        $normalized_text = str_replace( array("\r\n", "\r"), "\n", $default_text );
                                                        
                                                        // 2. Divide o texto onde houver "Enter duplo" (criando os Parágrafos)
                                                        $paragraphs = explode("\n\n", $normalized_text);
                                                        
                                                        $formatted_text = '';
                                                        foreach ($paragraphs as $p) {
                                                            // 3. Envolve cada parágrafo em uma div. Se houver um "Enter simples" dentro dele, vira <br>.
                                                            $formatted_text .= '<div>' . str_replace("\n", '<br>', $p) . '</div>';
                                                        }
                                                        
                                                        // 4. Garante que divs vazias tenham altura para não quebrar o clique
                                                        $formatted_text = str_replace('<div></div>', '<div><br></div>', $formatted_text);
                                                    } else {
                                                        $formatted_text = '';
                                                    }
                                                ?>
                                                 <?php 
                                                    $raw_align = ! empty( $field->alignment ) ? trim( $field->alignment ) : 'left';
                                                    // Converte variações para CSS padrão
                                                    if ( $raw_align === 'h-center' ) {
                                                        $align_style = 'center';
                                                    } else {
                                                        $align_style = esc_attr( $raw_align );
                                                    }
                                                ?>
                                                <div id="bcek_field_<?php echo esc_attr( $field->field_id ); ?>" class="bcek-rich-text-input w-full bg-white rounded-lg p-3 border border-gray-300 focus:ring-2 focus:ring-blue-500" style="height: 120px; overflow-y: auto; text-align: <?php echo $align_style; ?>;" contenteditable="true"><?php echo wp_kses_post( $formatted_text ); ?></div>
                                                
                                                <div class="pt-2">
                                                    <label class="block text-xs font-medium text-gray-500 mb-2">Tamanho da Fonte</label>
                                                    <div class="bcek-font-size-control">
                                                        <input type="number" class="bcek-dynamic-fontsize-input" value="<?php echo esc_attr( $field->font_size ); ?>" min="8" max="200">
                                                        <input type="range" class="bcek-dynamic-fontsize-slider" value="<?php echo esc_attr( $field->font_size ); ?>" min="8" max="200">
                                                    </div>
                                                </div>
