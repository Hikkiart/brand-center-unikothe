...
$categories = BCEK_Database::get_all_categories(); // Busca todas as categorias existentes

// Define um placeholder se não houver imagem base.
    ...
    <div class="flex flex-col md:flex-row gap-6 md:gap-8">
        ...
        <div id="editor-controls" class="w-full md:w-1/3 lg:w-1-4 p-4 md:p-6 bg-gray-50 rounded-2xl overflow-y-auto relative">
            <div class="space-y-6">
                ...
                <div>
                    <label for="template-category" class="block text-lg font-bold text-gray-800 mb-2">Categoria</label>
                    <select id="template-category" class="w-full bg-white rounded-lg p-3 border border-gray-300 focus:ring-2 focus:ring-blue-500">
                        <option value="">Nenhuma</option>
                        <?php if ( ! empty( $categories ) ) : ?>
                            <?php foreach ( $categories as $category ) : ?>
                                <option value="<?php echo esc_attr( $category->category_id ); ?>" <?php selected( $template_data->category_id ?? '', $category->category_id ); ?>>
                ...
                <div class="space-y-2">
                    ...
                    <button id="base-image-upload" class="w-full bg-white border border-gray-300 text-gray-700 font-semibold py-2 px-5 rounded-lg hover:bg-gray-100 transition-colors">
                        Carregar Imagem
                    </button>
            ...
            <button id="add-field-btn" class="mt-6 w-full bg-gray-100 text-gray-700 font-semibold py-2 px-5 rounded-lg hover:bg-gray-200 transition-colors flex items-center justify-center gap-2" style="display: none;">
