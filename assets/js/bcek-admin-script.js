jQuery(document).ready(function($) {
    ...
    // --- Drag and Drop ordem dos campos (SortableJS) ---
    ...
    /**
     * Inicializa o seletor de cores do WordPress (wpColorPicker).
     */
    ...
    // Lógica do upload/seleção de imagem base
    var frame;
    $('#bcek_base_image_button').on('click', function(e) {
        ...
        frame = wp.media({
            title: 'Selecionar Imagem Base',
            button: { text: 'Usar esta imagem' },
            multiple: false
        });
        frame.on('select', function() {
            var attachment = frame.state().get('selection').first().toJSON();
            $('#bcek_base_image_id').val(attachment.id);
            $('#bcek_base_image_url').val(attachment.url);
    ...
    $('#bcek_remove_base_image_button').on('click', function(e) {
        e.preventDefault();
        $('#bcek_base_image_id').val('');
        $('#bcek_base_image_url').val('');
        $('#bcek_admin_base_image_preview_img').attr('src', '').hide();
        if (typeof window.setupAdminCanvas === 'function') window.setupAdminCanvas();
        $(this).hide();
    });

    // Gestão dinâmica de campos
    var fieldNextIndex = $('#bcek_fields_container .bcek-field-item').length;
    var removedFieldIds = [];
    ...
    if ($adminPreviewArea.length) {
        ...
        window.drawAdminFieldsPreview = function() {
            if (!adminCtx) return;
            if ($adminCanvas.length) {
                adminCtx.clearRect(0, 0, $adminCanvas[0].width, $adminCanvas[0].height);
            }
            if (adminBaseImageNaturalWidth === 0 || adminBaseImageNaturalHeight === 0) return;
                ...
                if (fieldType === 'image') {
                    ...
                    if (textToRender !== undefined) {
                        ...
                        for (const line of lines) {
                            ...
                            if (line === '') { currentTextY += actualLineHeight; continue; }
