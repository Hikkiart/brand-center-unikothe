...
const BCEK_User_Events = {
    ...
    resizeTimeout: null, // Nova variável de controle para o resize
    ...
    init(state, ui, ajax) {
        ...
        // Otimização: Debounce no resize. O Canvas só será redesenhado 
        // 100 milissegundos após o usuário parar de redimensionar a janela.
        window.addEventListener('resize', () => {
            clearTimeout(this.resizeTimeout);
            this.resizeTimeout = setTimeout(() => {
                this.ui.redrawPreview();
            }, 100);
        });
    ...
    async onBaseImageLoad() {
        try {
            // Este comando espera que TODAS as fontes (incluindo as do Google Fonts)
            // estejam prontas para serem usadas, sem causar erros 404.
            await document.fonts.ready;
        } catch (e) {
            console.error('Erro ao aguardar pelas fontes:', e);
        }
        
        // Apenas depois de as fontes estarem prontas, atualiza os inputs e desenha o preview.
    ...
    handleFormInput(e) {
        ...
        if (target.matches('.bcek-dynamic-fontsize-slider, .bcek-dynamic-fontsize-input')) {
            const group = target.closest('.bcek-input-group');
            const slider = group.querySelector('.bcek-dynamic-fontsize-slider');
            const numberInput = group.querySelector('.bcek-dynamic-fontsize-input');

            if (target.matches('.bcek-dynamic-fontsize-slider')) {
        ...
        if (target.matches('.bcek-rich-text-input, .bcek-dynamic-fontsize-slider, .bcek-dynamic-fontsize-input')) {
