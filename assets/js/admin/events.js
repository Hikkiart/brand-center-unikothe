    ...
    handleSaveTemplate() {
        ...
        const fieldsArray = Object.values(this.state.fieldsState).map(field => {
            return {
                ...
                container_shape: field.container_shape || 'rectangle'
            };
        });
        ...
        // --- DEBUG NO CONSOLE ---
        // Mostra o objeto completo que estamos a enviar para o servidor.
        console.log('[BCEK DEBUG] A enviar os seguintes dados para o servidor:', dataToSend);
        
        jQuery.post(this.state.bcekData.ajax_url, dataToSend)
            .done(response => {
                // DEBUG: Mostra a resposta do servidor em caso de sucesso.
                console.log('[BCEK DEBUG] Resposta de sucesso do servidor:', response);
                
                if (response.success) {
                    alert(response.data.message || 'Template salvo com sucesso!');
                    if (response.data.is_new && response.data.redirect_url) {
            ...
            .fail((jqXHR, textStatus, errorThrown) => {
                // DEBUG: Mostra detalhes do erro em caso de falha na comunicação.
                console.error('[BCEK DEBUG] Falha na chamada AJAX! Status:', textStatus, 'Erro:', errorThrown);
                console.error('[BCEK DEBUG] Resposta completa do servidor (se houver):', jqXHR.responseText);
