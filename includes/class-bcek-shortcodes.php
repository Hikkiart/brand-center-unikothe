...
class BCEK_Shortcodes {
    ...
    public function render_editor_shortcode( $atts ) {
        ...
        if ( $template_id > 0 ) {
            ...
            // Passa os dados para o JavaScript (sem 'fonts_url' ou 'font_families')
            $editor_data = array(
                'template' => $template, 
                'fields'   => $fields,
            ...
            wp_enqueue_style('bcek-user-style');
            wp_enqueue_script('bcek-user-filter-script');
            
            $grouped_templates = BCEK_Database::get_templates_grouped_by_category();
