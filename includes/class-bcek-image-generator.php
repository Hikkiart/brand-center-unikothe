...
class BCEK_Image_Generator {
     ...
     */
    public static function generate( $template, $fields, $user_inputs, $user_filename = '', $format = 'png', $behind_overlay_data = '', $above_overlay_data = '' ) {
        ...
        $base_image_resource = self::create_image_resource_from_path($base_image_path, $image_info['mime']);
        if (is_wp_error($base_image_resource)) return $base_image_resource;

        // 2. Criar a tela final com fundo transparente
        $final_image = imagecreatetruecolor($image_info[0], $image_info[1]);
        imagesavealpha($final_image, true);
        $transparent_background = imagecolorallocatealpha($final_image, 0, 0, 0, 127);
        imagefill($final_image, 0, 0, $transparent_background);

        // --- NOVA LÓGICA DE MONTAGEM DE CAMADAS ---

        // 3. Desenhar a camada "DEBAIXO" primeiro
        if ( !empty($behind_overlay_data) ) {
            $behind_overlay = self::create_image_resource_from_data_url($behind_overlay_data);
            if ($behind_overlay) {
                imagecopy($final_image, $behind_overlay, 0, 0, 0, 0, $image_info[0], $image_info[1]);
                imagedestroy($behind_overlay);
            }
        }

        // 4. Desenhar a Imagem de Base no meio
        imagecopy($final_image, $base_image_resource, 0, 0, 0, 0, $image_info[0], $image_info[1]);
        imagedestroy($base_image_resource);

        // 5. Desenhar a camada "DE CIMA" por último
        if ( !empty($above_overlay_data) ) {
            $above_overlay = self::create_image_resource_from_data_url($above_overlay_data);
            if ($above_overlay) {
                imagecopy($final_image, $above_overlay, 0, 0, 0, 0, $image_info[0], $image_info[1]);
                imagedestroy($above_overlay);
            }
        }
        
        // --- FIM DA LÓGICA ---

        // 6. Salvar o ficheiro final
    ...
    private static function save_final_image($image_resource, $user_filename, $format) {
        ...
        if ($format === 'bmp' && function_exists('bcek_convert_png_to_bmp')) {
            ...
            if (imagepng($image_resource, $temp_png_path, 2)) {
                $save_success = bcek_convert_png_to_bmp($temp_png_path, $filepath);
                @unlink($temp_png_path);
            }
        } else {
            imagesavealpha($image_resource, true);
            $save_success = imagepng($image_resource, $filepath, 2);
        }
