...
function bcek_wrap_text_for_gd( $font_size, $angle, $font_file, $text, $max_width ) {
    ...
    foreach ($paragraphs as $paragraph) {
        ...
        foreach ($words as $word) {
            ...
            if ($bbox === false) { 
                error_log("BCEK wrap_text_gd: imagettfbbox falhou. Texto: '$test_line', Fonte: $font_file, Tamanho: $font_size");
...
function bcek_generate_image_with_gd( $template, $fields, $user_inputs, $user_filename = '', $format = 'png' ) {
    ...
    if ( $template && isset($template->base_image_id) && $template->base_image_id > 0 ) {
        ...
        if ( !$base_image_path || !file_exists( $base_image_path ) ) {
            return new WP_Error( 'base_image_file_not_found', __('Arquivo da imagem base não encontrado no servidor para o ID: ', 'bcek') . $template->base_image_id );
        }
    ...
    if ($format === 'bmp') {
        if (function_exists('bcek_convert_png_to_bmp')) {
            ...
            imagealphablending($image, false);
            imagesavealpha($image, true);
            $png_saved = @imagepng($image, $temp_png_filepath, 9);
            ...
            @imagedestroy( $image );
            return new WP_Error('bmp_function_missing', 'A função para criar BMP não foi encontrada.');
        }
    } else { 
        imagealphablending($image, false);
        imagesavealpha($image, true);
